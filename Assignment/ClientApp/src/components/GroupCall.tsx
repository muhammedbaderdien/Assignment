...
export default (props: GroupCallProps): JSX.Element => {
  ...
  const { callAgent, call, groupId, joinGroup, attempts, errorHandler, threadMembersError, getThread } = props;
  ...
  useEffect(() => {
    ...
    if (callAgent && !call) {
      joinGroup();
      document.title = `${groupId} group call`;
    }
        ...
        {!props.shareScreen ? (
          props.callState === Constants.CONNECTED && (
            <Stack horizontal styles={containerStyles}>
              ...
              {selectedPane !== SidePanelTypes.None && (
